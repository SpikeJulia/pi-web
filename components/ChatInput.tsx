...
interface Props {
  // Ticket 04: the send path now accepts the full pending-attachment list
  // so the parent can run the just-in-time upload pipeline (see
  // `useAgentSession.handleSend`). The second arg is only set when there
  // are chips pending — the no-attachments send path is identical to
  // before this ticket.
  onSend: (message: string, pending?: PendingAttachment[]) => Promise<"ok" | "error"> | void;
  onAbort: () => void;
...
export interface ChatInputHandle {
  ...
  /** Funnel for any file source (picker / drop / paste). Added in ticket 02. */
  addFiles: (files: File[]) => void;
  /**
   * Back-compat alias for `addFiles`, kept so external callers (drag-drop in
   * ChatWindow) and existing types keep compiling. New code should call
   * `addFiles` directly.
   */
  addImages: (files: File[]) => void;
  /** Ticket 04: surface an upload failure on the chips the failed batch
...
export const ChatInput = forwardRef<ChatInputHandle, Props>(function ChatInput({
  ...
  const bashMode = pendingAttachments.length === 0 && trimmedValue.startsWith("!");
  ...
  useImperativeHandle(ref, () => ({
    ...
    // Mark every chip with one of these ids as errored (red border +
    // tooltip). Called by the send pipeline when upload fails — the chips
    // stay in the list so the user can see which files failed and either
    // remove them or edit and retry.
    markAttachmentError(ids: string[], message: string) {
  ...
  /**
   * Funnel entry point for picker / drop / paste. Accepts any file type
   * and appends new chips into `pendingAttachments` (the single source of
   * truth for the send pipeline as of ticket 04). The legacy
   * `attachedImages` mirror was removed — image files no longer pay a
   * base64-encode penalty at attach time; the upload pipeline derives
   * base64 lazily on send and ships everything through one channel.
   * Rejected files (cap exceeded) surface as a notice.
   */
  ...
  const handleSend = useCallback(async () => {
    ...
    // Ticket 04: a chip in `error` status means the last send's upload
    // failed and we surfaced the server message on the chip. The user
    // must explicitly remove or retry the failing attachment(s) before
    // we touch the network again.
    const erroredChips = pendingAttachments.filter((a) => a.status === "error");
    if (erroredChips.length > 0) {
      const firstError = erroredChips.find((a) => a.error)?.error;
      showAttachmentError(
        firstError
          ? `Cannot send: ${erroredChips.length === 1 ? "attachment" : `${erroredChips.length} attachments`} failed to upload (${firstError}). Remove or retry first.`
          : `Cannot send: ${erroredChips.length === 1 ? "attachment" : `${erroredChips.length} attachments`} are in an error state. Remove or retry first.`,
      );
      return;
    }

    onAudioUnlock?.();
    if (!pendingAttachments.length && msg.startsWith("/") && onBuiltinCommand) {
    ...
    // The send pipeline is responsible for either uploading + dispatching
    // the prompt, or marking chips in error and not sending. We only
    // clear the input on a successful send — an upload failure must
    // leave the text and chips intact so the user can fix and retry.
    let sendResult: "ok" | "error";
    try {
      // The onSend prop may return a promise resolving to "ok" | "error",
      // or it may return void (e.g. a wrapped function that does not opt
      // into the new return convention yet). Treat a void/undefined
      // return as "ok" so existing one-off call sites keep working.
      const rawResult = await onSend(msg, pendingAttachments.length ? pendingAttachments : undefined);
      sendResult = rawResult === "error" ? "error" : "ok";
    } catch (e) {
      console.error("onSend threw:", e);
      sendResult = "error";
    }
    if (sendResult === "error") {
      // Keep the text and chips for retry; just surface a banner so the
      // user knows the send did not go through.
      showAttachmentError("Send was aborted. Check the attachments above and try again.");
      return;
    }
    clearInput();
  }, [value, pendingAttachments, isStreaming, onBuiltinCommand, onSend, clearInput, onAudioUnlock, showAttachmentError]);
  ...
  const canQueueStreamingMessage = hasInputText && pendingAttachments.length === 0;
  ...
  const sendQueued = useCallback((mode: "steer" | "followup") => {
    const msg = value.trim();
    if (!msg && !pendingAttachments.length) return;
    // Ticket 04: queued (steer / follow-up) sends cannot carry
    // attachments because the upload pipeline only runs in handleSend.
    ...
    clearInput();
  }, [value, pendingAttachments, onPromptWithStreamingBehavior, onSteer, onFollowUp, clearInput, onAudioUnlock]);
  ...
  const handlePaste = useCallback((e: React.ClipboardEvent) => {
    // Only image clipboard items become attachments — pasting text or files
    // from the OS clipboard stays the textarea's job. Ctrl+V / Alt+V on a
    // screenshot lands here as one or more `image/png` items.
  ...
  return (
    ...
    >
      {/* Hidden file input — accepts any file type, multi-select. Drives the
          paperclip button; the picker is hidden so it never blocks the layout. */}
      <input
        ref={fileInputRef}
        type="file"
        multiple
      ...
      <div style={{ maxWidth: 820, margin: "0 auto" }}>
        ...
        {/* Attachment chips + inline error banner. Replaces the old image-only
            preview row so any file type renders consistently; image files still
            get a 56px thumbnail, non-images render the type icon + name + size. */}
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      alignItems: "center",
                      gap: 6,
                      padding: isImg ? "2px 22px 2px 2px" : "3px 22px 3px 6px",
                      maxWidth: 240,
                    ...
                    data-attachment-name={att.file.name}
                    title={`${att.file.name} · ${formatAttachmentSize(att.file.size)}${att.error ? ` · ${att.error}` : ""}`}
                  >
                    {isImg ? (
                      ...
                      <img
                        src={att.previewUrl}
                        alt=""
                        style={{ width: 28, height: 28, objectFit: "cover", borderRadius: 5, flexShrink: 0, display: "block" }}
                      />
                    ) : (
                      <span style={{ display: "flex", alignItems: "center", flexShrink: 0 }}>
                        {getFileIcon(att.file.name, 14)}
                      </span>
                    )}
                    <span style={{ minWidth: 0, display: "flex", flexDirection: "column", gap: 1, lineHeight: 1.2 }}>
                      <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        {att.file.name}
                      </span>
                      <span style={{ fontSize: 10, color: "var(--text-dim)", fontFamily: "var(--font-mono)" }}>
                        {formatAttachmentSize(att.file.size)}
                      </span>
                    </span>
                    <button
                      ...
                      style={{
                        position: isImg ? "absolute" : "static",
                        top: isImg ? -4 : undefined,
                        right: isImg ? -4 : undefined,
                        marginLeft: isImg ? 0 : "auto",
          ...
          {isStreaming ? (
            <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0, alignSelf: "flex-end" }}>
              {onSteer && (
                <button
                  ...
                  title={pendingAttachments.length ? "Attachments cannot be queued while the agent is running" : "Interrupt the current run and inject this message now"}
                  style={{
              ...
              {onFollowUp && (
                <button
                  ...
                  disabled={!canQueueStreamingMessage}
                  title={pendingAttachments.length ? "Attachments cannot be queued while the agent is running" : "Queue this message after the agent finishes"}
                  style={{
                    display: "flex", alignItems: "center", gap: 5,
                    padding: "7px 12px",
            ...
            <button
              ...
              style={{
                ...
                display: "flex", alignItems: "center", gap: 6,
                padding: "7px 14px",
                background: (value.trim() || pendingAttachments.length) ? "var(--accent)" : "var(--bg-panel)",
                border: "none",
                borderRadius: 8,
                color: (value.trim() || pendingAttachments.length) ? "#fff" : "var(--text-dim)",
                cursor: (value.trim() || pendingAttachments.length) ? "pointer" : "not-allowed",
                fontSize: 13,
                fontWeight: 600,
                letterSpacing: "-0.01em",
                boxShadow: (value.trim() || pendingAttachments.length) ? "0 1px 3px rgba(37,99,235,0.25)" : "none",
        ...
        <div style={{
          ...
          <div style={{ flex: isMobile ? "1 1 auto" : "0 0 auto", minWidth: 0, display: "flex", alignItems: "center", gap: 2 }}>
            <button
              ...
              disabled={isStreaming}
              title={`Attach files (up to ${MAX_ATTACHMENTS_PER_MESSAGE} per message)`}
              ...
              style={{
                ...
                borderRadius: 9,
                color: pendingAttachments.length ? "var(--accent)" : "var(--text-muted)",
              ...
              onMouseEnter={(e) => {
                if (isStreaming) return;
                e.currentTarget.style.background = "var(--bg-hover)";
                e.currentTarget.style.color = pendingAttachments.length ? "var(--accent)" : "var(--text)";
              }}
              onMouseLeave={(e) => {
                e.currentTarget.style.background = "none";
                e.currentTarget.style.color = pendingAttachments.length ? "var(--accent)" : "var(--text-muted)";
              }}
            >
              {/* Paperclip icon — replaces the image-only photo icon so any
                  file type is discoverable. */}
