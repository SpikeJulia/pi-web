...
import { AttachmentsConfig } from "./AttachmentsConfig";
...
export function AppShell() {
  ...
  const [attachmentsConfigOpen, setAttachmentsConfigOpen] = useState(false);
  ...
  const settingsButtonCount = effectiveProjectCwd ? 5 : 3;
    ...
    <>
      ...
      <div style={{ padding: "8px", flexShrink: 0, display: "flex", justifyContent: "space-between", gap: compactSettingsButtons ? 2 : 4 }}>
        {([
          ...
          ...(effectiveProjectCwd ? [{
            label: "Attachments",
            onClick: () => setAttachmentsConfigOpen(true),
            disabled: false,
            icon: (
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    ...
    {attachmentsConfigOpen && effectiveProjectCwd && (
      <AttachmentsConfig
        cwd={effectiveProjectCwd}
        sessionId={selectedSession?.id ?? null}
        onClose={() => setAttachmentsConfigOpen(false)}
      />
    )}
