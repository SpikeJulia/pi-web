...
test("parseSidecarPayload accepts /api/files read responses with JSON content", () => {
  const result = parseSidecarPayload({
    content: JSON.stringify(VALID_META),
    language: "json",
    size: JSON.stringify(VALID_META).length,
  });
  assert.deepEqual(result, VALID_META);
});

test("parseSidecarPayload rejects invalid wrapped JSON content", () => {
  assert.equal(parseSidecarPayload({ content: "not json" }), null);
});
...
test("fetchSidecarMeta returns parsed metadata from /api/files read wrappers", async () => {
  const meta = await fetchSidecarMeta(CWD, SESSION, STORED, async () => okJson({
    content: JSON.stringify(VALID_META),
    language: "json",
    size: JSON.stringify(VALID_META).length,
  }));
...
test("getFallbackDisplayName hides random stored names", () => {
  assert.equal(getFallbackDisplayName(STORED), "PDF attachment");
  assert.equal(getFallbackDisplayName("noext"), "Attachment");
});