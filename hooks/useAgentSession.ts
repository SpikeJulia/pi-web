...
export interface ChatInputHandle {
  ...
  /** Mark the chips with the given ids as errored (red border, error tooltip).
   *  Used by the send pipeline to surface upload failures in-place. */
...
export function useAgentSession(opts: UseAgentSessionOptions) {
  ...
  const handleSend = useCallback(async (
    ...
    if (!trimmedMessage && !pending?.length) return "error";
    if (agentRunningRef.current || bashRunningRef.current) return "error";

    const hasAttachments = (pending?.length ?? 0) > 0;
    const isSlashCommandPrompt = !hasAttachments && trimmedMessage.startsWith("/");

    const isBashCommand = !hasAttachments && trimmedMessage.startsWith("!");
    if (isBashCommand) {
      const isExcluded = trimmedMessage.startsWith("!!");
      const bashCmd = (isExcluded ? trimmedMessage.slice(2) : trimmedMessage.slice(1)).trim();
      if (!bashCmd) return "error";
    ...
    // ---- Just-in-time upload + composition ------------------------------
    // When the user attached files, send the prompt text through the pure
    // `composeAttachmentMessage` helper once we know the server's stored
    // paths. Image-channel attachments stay in the `images` field with no
    // path in text (ADR 0004); path-channel attachments append @path
    ...
    if (hasAttachments && pending) {
      // Decide cwd up front — if we have no cwd, attachments cannot be
      // uploaded and there is nothing else to do but surface the issue.
      const cwd: string = (session?.cwd ?? (isNew ? newSessionCwd : null) ?? "");
      if (!cwd) {
        const err = "Cannot upload attachments: project path is unknown.";
        chatInputRef?.current?.markAttachmentError(pending.map((p) => p.id), err);
        addNotice({ type: "error", message: err });
        return "error";
      }

      // For new sessions we need a session id *before* we can upload
      // (the route stores under .pi-uploads/<sessionId>/). Reuse the
      // existing ensureNewSession path so the new-session session id
      // becomes available up-front; the same id is then used for the
      // prompt command below, matching the ticket requirement that the
      // session is created just-in-time on send.
      let sid: string | null = null;
      try {
        if (isNew && newSessionCwd) {
          const existingSid = sessionIdRef.current ?? await ensuringNewSessionRef.current;
          sid = existingSid ?? (await ensureNewSession()) ?? null;
        } else if (session) {
          sid = session.id;
        }
      } catch (e) {
        console.error("Failed to prepare session for upload:", e);
      }
      if (!sid) {
        const err = "Cannot upload attachments: session is not ready.";
        chatInputRef?.current?.markAttachmentError(pending.map((p) => p.id), err);
        addNotice({ type: "error", message: err });
        return "error";
      }

      const uploadResult = await uploadAttachments(cwd, sid, pending.map((p) => p.file));
      if (!uploadResult.ok) {
        // Mark every chip in error so the user can see exactly which
        // attachments failed, and surface the server-provided message
        // verbatim. The text and chips stay for retry; nothing has been
        // sent to the agent yet.
        chatInputRef?.current?.markAttachmentError(pending.map((p) => p.id), uploadResult.error);
        addNotice({ type: "error", message: uploadResult.error });
        return "error";
      }

      composed = composeAttachmentMessage(message, pending, uploadResult.files);
    }

    // ---- Optimistic user bubble -----------------------------------------
    const piImages = composed.images;
    ...
    const userMsg: AgentMessage = {
      ...
      content: imageBlocks.length
        ? [
            ...(composed.message ? [{ type: "text" as const, text: composed.message }] : []),
            ...imageBlocks,
          ]
        : composed.message,
      timestamp: Date.now(),
    ...
    try {
      ...
      if (isNew && newSessionCwd) {
        ...
        if (sid) {
          ...
          promoteNewSession(1, composed.message);
        }
      } else if (session) {
  ...
  }, [isNew, newSessionCwd, newSessionModel, session, ensureNewSession, ensureEventsConnected, promoteNewSession, waitForPromptSettlement, addNotice, chatInputRef]);
